...
- **Интеграция с заметками (Markdown)**:
  - Автодополнение: наберите `@remind` в любой заметке для вызова удобного меню выбора даты.
  - Кастомный Datepicker: возможность выбрать любую точную дату и время через встроенный календарь.
  - Фоновый сканер: напоминания из текста автоматически подхватываются и отображаются в боковой панели.
  - Автозавершение: после срабатывания текстового напоминания, плагин сам заменит тег на `@remind-done(...)` в файле.
  - Умная навигация: клик по уведомлению такого напоминания откроет нужную заметку на нужной строке.
...
│   ├── markdownScanner.ts   # Фоновый сканер для поиска тегов @remind в файлах
│   ├── MarkdownSuggest.ts   # Автодополнение (EditorSuggest) для ввода @remind
│   ├── CustomDateModal.ts   # Кастомный календарь для выбора даты в тексте
